...
const PlantTicketTable = () => {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      const response = await http.get(
        `/tickets/parent/${parent_id}/${department}?query=${query ? query : "all"}`,
      );
  ...
  const handleTicketClick = (id) => {
    ...
    navigate(`/plant-supervisor/dashboard/ticket-details/${id}`);
  };

  const [updatingId2, setUpdatingId2] = useState(null);
  const [updatingId3, setUpdatingId3] = useState(null);
  const [updatingId4, setUpdatingId4] = useState(null); // To track which ticket is updating

  // To track which ticket is updating
...
export default PlantTicketTable;
